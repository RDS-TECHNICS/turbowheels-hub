...
async function sendEmail(to, subject, html, text = '') {
  ...
  try {
    const mailOptions = {
      from: `"TURBO WHEELS" <${process.env.FROM_EMAIL}>`,
      to,
      subject,
      text,
      html
...
app.post('/api/forgot-password', async (req, res) => {
  try {
    ...
    try {
      ...
      const mailOptions = {
       from: `"TURBO WHEELS Security" <${process.env.FROM_EMAIL}>`,
       to: email,
        subject: 'Reset Your Password - TURBO WHEELS',
        text: `
          ...
          We received a request to reset the password for your Soul Art Studio account.
          ...
          1. Enter the 6-digit OTP on the password reset page
          2. Create a new strong password for your account
          3. Sign in with your new password to access your account
      ...
      const info = await transporter.sendMail(mailOptions);
      console.log('✅ Professional OTP email sent successfully:', info.messageId);
